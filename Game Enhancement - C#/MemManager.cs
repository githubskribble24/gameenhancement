...
namespace Game_Enhancement___C_
{
    public class MemManager
    {
        ...
        public MemManager(string procname)
        {
            GetProcess(procname);
        }
        ...
        public Process GetProcess(string procname)
        {
            ...
            {
                Console.WriteLine("Out of range. Could not find the process: {0}", procname);
                Environment.Exit(2);
                return proc;
            }
        }
        ...
        public byte[] ReadBytes(IntPtr addy, int bytes)
        {
            ...
            ReadProcessMemory(proc.Handle, addy, buffer, bytes, IntPtr.Zero);
            return buffer;
        }
        ...
        {
            byte[] buffer = new byte[bytes];
            ReadProcessMemory(proc.Handle, addy + offset, buffer, bytes, IntPtr.Zero);
            return buffer;
        }
        ...
        {
            return BitConverter.ToDouble(ReadBytes(address, 4));
        }
        ...
        {
            // float = 4 bytes, 4*3 = 12
            // we need to read 12 bytes
        ...
        {
            // a float is 4 bytes
            // a matrix is 4 rows and 4 columns = 4*4 = 16
            var bytes = ReadBytes(address, 16*4);
            float[] matrix = new float[bytes.Length];
        ...
        public bool WriteInt(IntPtr address, int value, int offset)
        {
        ...
        public bool WriteShort(IntPtr address, short value, int offset)
        {
            return WriteBytes(address+offset, BitConverter.GetBytes(value));
        }
        ...
        public bool WriteUShort(IntPtr address, ushort value, int offset)
        {
            return WriteBytes(address + offset, BitConverter.GetBytes(value));
        }
        ...
        public bool WriteString(IntPtr address, int offset, string value)
        {
            return WriteBytes(address+offset, Encoding.UTF8.GetBytes(value));
        }
